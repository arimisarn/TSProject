...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let livres = livresData ? JSON.parse(livresData) : [];
    // Filtrer les livres pour obtenir uniquement ceux de l'utilisateur connecté
    const userLivres = livres.filter((livre) => livre.auteur === auteurConnecte);
    if (userLivres.length === 0) {
        ...
        userLivres.forEach((livre, index) => {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                if (indexToDelete !== undefined) {
                    ...
                    let tousLesLivres = tousLesLivresData ? JSON.parse(tousLesLivresData) : [];
